...
def tavily_search(api_key: str, query: str, count: int = 5, include_domains: list[str] | None = None, timeout: int = 45) -> list[dict]:
    ...
    response = requests.post(
        TAVILY_ENDPOINT,
        headers={"Accept": "application/json", "Content-Type": "application/json", "User-Agent": "ResetLeadFinder/8.0"},
        json=payload,
        timeout=timeout,
    )
...
def fetch_public_site_emails(domain: str) -> tuple[set[str], list[str]]:
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; ResetLeadFinder/8.0; public-contact-research)"}
    for path in paths:
...
def abstract_verify(api_key: str, email: str, timeout: int = 30) -> dict:
    ...
    response = requests.get(
        ...
        headers={"Accept": "application/json", "User-Agent": "ResetLeadFinder/8.0"},
        timeout=timeout,
    )
    if response.status_code in (401, 403):
...
st.set_page_config(page_title="Reset Lead Finder", page_icon="🔎", layout="wide")
...
def require_login() -> None:
    ...
    if st.session_state.get("reset_authenticated"):
        return
    st.markdown("## Reset Lead Finder")
    st.caption("Yetkili erişim")
    password = st.text_input("Uygulama şifresi", type="password", key="login_password")
...
st.title("Reset Lead Finder · Cloud")
st.caption("Şirketlerden hedef kişileri bulur, kurumsal e-posta adaylarını üretir, doğrular ve nihai CRM Excel’i hazırlar.")
